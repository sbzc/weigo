...
public class TbUserLoginServiceImpl implements TbUserLoginService {
	...
	@Override
	public VisitorPage selectVisitorPageByDate(Date date) {
		...
		calendar.set(Calendar.MILLISECOND, 0);//ºÁÃë
		...
		for(int i=0;i<7;i++) {
			...
			v.setCount(count);
			v.setNamePage(calendar.get(Calendar.MONDAY)+1+"ÔÂ"+calendar.get(Calendar.DAY_OF_MONTH)+"ÈÕ");
			calendar.set(Calendar.DAY_OF_MONTH,calendar.get(Calendar.DAY_OF_MONTH)+1);
			lsitValue.add(v);
		}
		visitorPage.setValues(lsitValue);
		return visitorPage;
	}
