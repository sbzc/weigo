...
public class TbItemServiceImpl implements TbItemService {
   ...
   private TbUserDubboService tbUserDubboService;
	...
	public int deleteItemByIds(String ids) {
		...
		new Thread() {
			public void run() {
				 HttpClientUtil.doPost("http://47.103.218.192:8083/solr/delete?ids="+ids);
			};
		}.start();
		}
		return row;
	...
	public int updateItemByTbItem(String ids,int status) {
		...
		if(status==0) {
			...
			new Thread() {
				public void run() {
					 HttpClientUtil.doPost("http://47.103.218.192:8083/solr/insertItems?ids="+ids);
				};
			}.start();
		}
	...
	public int insertItemByItemAndDesc(TbItem item, String desc) {
		...
		new Thread() {
			public void run() {
				HttpClientUtil.doPostJson("http://47.103.218.192:8083/solr/insert", JsonUtils.objectToJson(itemFinal));
			};
		}.start();
		return ok;
	}
